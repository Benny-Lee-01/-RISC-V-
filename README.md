...
2. 將 `C:\Users\User\Desktop\riscv\src\hardware\` 底下的所有 Verilog 原始檔（`top.v`, `picorv32.v`, `timer.v`, `controller.v`）以及 `C:\Users\User\Desktop\riscv\constraints\basys3.xdc` 約束檔匯入專案。
3. 將編譯好的記憶體映像檔 `C:\Users\User\Desktop\riscv\src\firmware\firmware.mem` 作為設計來源一同併入專案資料夾，以供 BRAM 在合成時自動初始化讀取。
...
1. 進入 `C:\Users\User\Desktop\riscv\src\firmware\` 目錄，根據需求修改 `main.c` 中的控制邏輯或狀態機（如自訂暫停行為或 BCD 轉換演算法）。
...
* **系統啟動與初始化**: 燒錄完成後，頂層硬體模組會自動發送一次開機自動重置（Power-On Reset）訊號，系統直接跳過手動重置流程並即時讀取下方的 DIP 指撥開關。
* **自訂初始時間**: 透過撥動板上 16 個指撥開關 `SW[15:0]` 輸入目標秒數（採用二進位編碼），此時上方的七段顯示器會即時將其轉換為十進位制並顯示（最高支援 9999 秒）。
